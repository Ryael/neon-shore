...
A similar approach with stand-ups with the client was taken due to how it keeps them informed of progress each day, with sprints lasting between a day to two. All user stories were split into epics, which were then neatly organised and evaluated in terms of story points.
...
The biggest takeaway I encountered while working on this project is two-fold:
...
Below is a summary of learnings from each individual sprint, taken from screenshots of the sprint prior to completion, and my notes.
...
- Learning how to manipulate GSAP was relatively straight forward due to how expansive and detailed its documentation is. This allowed for a very easy implementation of the desired timing of the painting effect.
...
- A total of five bugs were discovered but four were resolved. The remaining one will be solved during future sprints as priority is complete an MVP.
...
- This sprint began on 13/07/23 at 21:31 and ended on 15/07/23 at 1:30. The estimated story point total was 7 at first but bugs were brought into it bringing the total to 20. All these issues were resolved in time, although it proved to be far longer than anticipated. While still a success, the remove button's bug in the missing or invalid CSRF error caused me a lot of trouble.
...
- Overall, the time spent was approximately 18 hours and matched the assigned story points.
...
- The greatest difficulty with this sprint was finding an alternative to Bootstrap's Toast system and finding a way of integrating it with Django. SweetAlerts were chosen as the flash message method of choice, and it took repeat read-throughs of the documentation to successfully implement it. At first, a center-screen modal pop-up approach was employed but then was later changed to be a much less distracting Toast-like notification.
- The special characters not rendering properly bug was resolved by replacing the `text` field with a `html` field, although this allows for XSS attacks on the website. However, as this website won't be deployed for a commercial purpose until much later, this was deemed a satisfactory compromise. This, will, however be dealt with in the future with input sanitation.
...
- 3 bugs were discovered while working on implementing Stripe functionality, and while not directly related to it, they were still resolved as they proved to be quite simple, and Stripe even helped understand why the delivery cost was not being displayed correctly.
...
- Three bugs were resolved that were found much earlier on, with the most notorious being the one that prevented emails from being sent. Tutor support was contacted and after a lot of troubleshooting the error was found: a superfluous newline in the subject template for the confirmation email. Tutor support's help was crucial in solving this and showed me how to troubleshoot errors with webhooks and how complicated it can be.
